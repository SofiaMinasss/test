...
class Main(tk.Frame):
    ...
    # добавление данных
    def records(self, name, tel, email, zar):
        self.db.insert_data(name, tel, email, zar)
        self.view_records()

    # обновление данных
    def update_record(self, name, tel, email, zar):
        self.db.c.execute('''UPDATE db SET name=?, tel=?, email=?, zar=? WHERE ID=?''', (name, tel, email, zar, self.tree.set(self.tree.selection()[0], '#1')))
        self.db.conn.commit()
        self.view_records()
...
class Child(tk.Toplevel):
    ...
    def init_child(self):
        ...
        self.btn_ok.bind('<Button-1>', lambda event: self.view.records(self.entry_name.get(), self.entry_email.get(), self.entry_tel.get(), self.entry_zar.get()))
...
class Update(Child):
    ...
    def init_edit(self):
        ...
        btn_edit.bind('<Button-1>', lambda event: self.view.update_record(self.entry_name.get(), self.entry_email.get(), self.entry_tel.get(), self.entry_zar.get()))
...
class DB:
    def __init__(self):
        ...
        self.c.execute('''CREATE TABLE IF NOT EXISTS db (id integer primary key, name text, tel text, email text, zar text)''')
        # сохранение изменений БД
        self.conn.commit()

    # метод добавления в БД
    def insert_data(self, name, tel, email, zar):
        self.c.execute('''INSERT INTO db (name, tel, email, zar) VALUES (?, ?, ?, ?)''', (name, tel, email, zar))
        self.conn.commit()
